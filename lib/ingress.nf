...
/**
 * Simple FASTA ingress for single genome assembly files.
 *
 * This ingress accepts a single FASTA file and creates a minimal metadata map.
 * Unlike fastq_ingress and xam_ingress, this does not support:
 ...
 *   - input: path to a single FASTA file (required)
 * @return: channel of [metamap, fasta_file_path]
 */
workflow fasta_ingress {
    take:
        arguments
    main:
        // Parse arguments
        def margs = parse_fasta_arguments(arguments)

        // Validate input file exists
        Path input_file
        try {
            input_file = file(margs.input, checkIfExists: true)
        } catch (NoSuchFileException e) {
            error "Input FASTA file '$margs.input' does not exist."
        }

        // Check it's a file, not a directory
        if (!input_file.isFile()) {
            error "Input '$input_file' must be a file, not a directory."
        }

        // Validate FASTA extension
        def valid_extensions = ['.fasta', '.fna', '.fa']
        def has_valid_ext = valid_extensions.any { ext ->
            input_file.name.toLowerCase().endsWith(ext) ||
            input_file.name.toLowerCase().endsWith(ext + '.gz')
        }

        if (!has_valid_ext) {
            error "Input file must have one of these extensions: ${valid_extensions.join(', ')}"
        }

        // Create metadata from filename (basename without extension)
        def alias = input_file.simpleName
        // Remove .gz if present
        if (alias.endsWith('.gz')) {
            alias = alias.replaceAll(/\.gz$/, '')
        }
        // Remove common FASTA extensions
        alias = alias.replaceAll(/\.(fasta|fna|fa)$/, '')

        // Create metamap with just the alias
        def meta = create_metamap([alias: alias])

        // Create channel
        ch_fasta = Channel.of([meta, input_file])
...
/**
 * Parse input arguments for fasta_ingress.
 *
 * @param arguments: map with input arguments
 * @return: map of parsed arguments
 */
Map parse_fasta_arguments(Map arguments) {
    ArrayList required_args = ["input"]
    Map default_kwargs = [:]
